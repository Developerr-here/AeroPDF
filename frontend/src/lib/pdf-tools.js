...
export async function htmlToPDF(payload) {
  ...
  const res = await authenticatedFetch('/api/html-to-pdf', {
    method: 'POST',
...
export async function comparePDFs(fileA, fileB) {
  ...
  const res = await authenticatedFetch('/api/compare', { method: 'POST', body: formData });
...
export async function aiAssistantPDF(file, mode, options = {}, token) {
  ...
  if (options.language) {
    formData.append('targetLanguage', options.language);
  }
  const headers = {};
  if (token) {
...
export async function aiRemoveBackground(file, token) {
  ...
  const res = await authenticatedFetch('/api/image/remove-background', {
    method: 'POST',
    headers: headers,
    body: formData
  });
...
export async function aiUpscaleImage(file, factor, token) {
  ...
  const res = await authenticatedFetch('/api/image/upscale', {
    method: 'POST',
    headers: headers,
    body: formData
  });
